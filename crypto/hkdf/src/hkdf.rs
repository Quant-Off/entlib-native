...
/// HKDF-SHA256의 해시 출력 크기 (바이트)
pub const SHA256_HASH_LEN: usize = 32;
/// RFC5869에 따른 OKM 최대 출력 크기 제한 (255 * HashLen)
pub const MAX_OKM_LEN: usize = 255 * SHA256_HASH_LEN;
...
impl HKDFSHA256 {
    ...
    pub fn extract(&self, salt: Option<&[u8]>, ikm: &[u8]) -> Result<SecureBuffer, HKDFState> {
        let default_salt = [0u8; SHA256_HASH_LEN];
        let actual_salt = salt.unwrap_or(&default_salt);

        // HMACSHA256을 이용한 PRK 계산
        let mut hmac = HMACSHA256::new(actual_salt).unwrap();
        hmac.update(ikm);
    ...
    pub fn expand(
        ...
        if length > MAX_OKM_LEN || okm.len() < length {
            return Err(HKDFState::InvalidLength);
        }
        if prk.len() < SHA256_HASH_LEN {
            return Err(HKDFState::InvalidLength);
        }

        let mut t = [0u8; SHA256_HASH_LEN];
