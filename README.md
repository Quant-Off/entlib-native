...
> 기술적 설명은 [퀀트 팀 공개 문서](https://docs.qu4nt.space/docs/projects/entanglementlib/entlib-native) 에서 확인할 수 있습니다.
...
네이티브 라이브러리의 `1.1.0` 릴리즈는 얽힘 라이브러리의 `1.1.0` 업데이트 처럼 아주 큰 변화를 가질 예정입니다. 큰 변경에 따라 자연스럽게 얽힘 라이브러리에도 큰 변화가 있을 예정입니다. 그렇게 릴리즈를 출시하고 나면, 두 라이브러리 모두 매우 같은 릴리즈 버전을 가지게 됩니다.

# 영감

마침 존경하는 보안 단체 `Legion of the BouncyCastle Inc`는 [`bc-rust`](https://github.com/bcgit/bc-rust/) 개발을 시작했고 얽힘 라이브러리 브릿징 기술에 유용할 법 한 영감을 많이 얻었습니다. 제 이슈나 PR을 읽어주진 않고 계시지만, 네... 그럼에도 불구하고 이들은 언제나 저의 힘이 되어주고 있습니다. 
...
이 네이티브 라이브러리의 벤치마킹은 `criterion` 크레이트를 통해 진행됩니다. 자세한 벤치마킹 결과는 [이 곳](BENCHMARK.md)에서 확인하실 수 있습니다.