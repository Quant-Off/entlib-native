...
mod kcmvp_cavp_hmac_sha3_test {
    ...
    //
    #[test]
    fn cavp_hmac_sha3_test() {
        use entlib_native_hmac::{HMACSHA3_224, HMACSHA3_256, HMACSHA3_384, HMACSHA3_512};
        ...
        let more = format!("{}/entanglementlib__CAVP_5_20260315192802", dir);

        process_cavp_file(
            format!("{}/HMAC_SHA3-224_KAT.req", more).as_str(),
            format!("{}/HMAC_SHA3-224_KAT.rsp", more).as_str(),
            |key, msg| {
                // 초기화 시 보안 정책(112bit 미만 차단 등)에 의해 에러가 발생할 수 있음
                let mut hmac = HMACSHA3_224::new(key).map_err(|_| "HmacInitError")?;
                hmac.update(msg);
                let result = hmac.finalize().map_err(|_| "HmacFinalizeError")?;
        ...
        process_cavp_file(
            format!("{}/HMAC_SHA3-256_KAT.req", more).as_str(),
            format!("{}/HMAC_SHA3-256_KAT.rsp", more).as_str(),
            |key, msg| {
                // 초기화 시 보안 정책(112bit 미만 차단 등)에 의해 에러가 발생할 수 있음
                let mut hmac = HMACSHA3_256::new(key).map_err(|_| "HmacInitError")?;
                hmac.update(msg);
                let result = hmac.finalize().map_err(|_| "HmacFinalizeError")?;
        ...
        process_cavp_file(
            format!("{}/HMAC_SHA3-384_KAT.req", more).as_str(),
            format!("{}/HMAC_SHA3-384_KAT.rsp", more).as_str(),
            |key, msg| {
                // 초기화 시 보안 정책(112bit 미만 차단 등)에 의해 에러가 발생할 수 있음
                let mut hmac = HMACSHA3_384::new(key).map_err(|_| "HmacInitError")?;
                hmac.update(msg);
                let result = hmac.finalize().map_err(|_| "HmacFinalizeError")?;
        ...
        process_cavp_file(
            format!("{}/HMAC_SHA3-512_KAT.req", more).as_str(),
            format!("{}/HMAC_SHA3-512_KAT.rsp", more).as_str(),
            |key, msg| {
                // 초기화 시 보안 정책(112bit 미만 차단 등)에 의해 에러가 발생할 수 있음
                let mut hmac = HMACSHA3_512::new(key).map_err(|_| "HmacInitError")?;
                hmac.update(msg);
                let result = hmac.finalize().map_err(|_| "HmacFinalizeError")?;
