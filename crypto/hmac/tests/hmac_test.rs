use entlib_native_hmac::{HmacError, MacResult, HMACSHA256, HMACSHA512};
...
fn sha256_tc6_key_longer_than_block() {
    assert_eq!(
        hmac256(&[0xaau8; 131], &[b"Test Using Larger Than Block-Size Key - Hash Key First"]),
        [
...
fn sha512_tc6_key_longer_than_block() {
    assert_eq!(
        hmac512(&[0xaau8; 131], &[b"Test Using Larger Than Block-Size Key - Hash Key First"]),
        [
...
fn rejects_empty_key_sha256() {
    assert!(matches!(HMACSHA256::new(&[]), Err(HmacError::WeakKeyLength)));
}

#[test]
fn rejects_empty_key_sha512() {
    assert!(matches!(HMACSHA512::new(&[]), Err(HmacError::WeakKeyLength)));
}
...
fn sha256_streaming_across_block_boundary() {
    ...
    for split in [63, 64, 65, 128] {
        assert_eq!(
            ...
            "SHA-256: {split}/{} 분할 불일치", 200 - split
        );
    }
}
...
fn sha512_streaming_across_block_boundary() {
    ...
    for split in [127, 128, 129, 256] {
        assert_eq!(
            ...
            "SHA-512: {split}/{} 분할 불일치", 400 - split
        );
    }
}
...
fn sha256_mac_ct_ne_different() {
    let key = [0x0bu8; 20];
    let mut h1 = HMACSHA256::new(&key).unwrap(); h1.update(b"a"); let m1 = h1.finalize().unwrap();
    let mut h2 = HMACSHA256::new(&key).unwrap(); h2.update(b"b"); let m2 = h2.finalize().unwrap();
    assert!(m1 != m2);
}
...
fn sha512_mac_ct_ne_different() {
    let key = [0x0bu8; 20];
    let mut h1 = HMACSHA512::new(&key).unwrap(); h1.update(b"a"); let m1 = h1.finalize().unwrap();
    let mut h2 = HMACSHA512::new(&key).unwrap(); h2.update(b"b"); let m2 = h2.finalize().unwrap();
    assert!(m1 != m2);
}
...
fn different_keys_different_macs() {
    ...
    assert_ne!(hmac256(&[0x01u8; 20], &[data]), hmac256(&[0x02u8; 20], &[data]));
    assert_ne!(hmac512(&[0x01u8; 20], &[data]), hmac512(&[0x02u8; 20], &[data]));
}
