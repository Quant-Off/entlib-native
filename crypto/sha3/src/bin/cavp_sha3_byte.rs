//! 얽힘 라이브러리(EntanglementLib) entlib-native 네이티브 SHA3 제품군 통합 Byte-Oriented & Monte Carlo CAVP 검증 도구
//! 지원 알고리즘: SHA3_224, SHA3_256, SHA3_384, SHA3_512
//!
//! 모든 입력 데이터의 Len이 8의 배수(Byte-aligned)임을 전제로 동작함
...
fn main() -> io::Result<()> {
    ...
    while i < lines.len() {
        ...
        if line.starts_with("Len = ") {
            ...
            // 바이트 정렬이 보장되므로 단순 update & finalize 로직 사용
            ...
            if !expected_md.is_empty() {
                ...
                if computed_md == expected_md {
                    ...
                    eprintln!("FAIL  Len = {} bits ({} bytes)", len, len / 8);
            ...
            // 다음 COUNT 루프를 위한 Seed 상태 연쇄(Chain)
